  ...
  <div className="grid min-h-screen bg-paper lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1fr)]">
    ...
    <aside className="relative isolate flex flex-col justify-between overflow-hidden bg-crimson-deep px-6 py-6 text-white sm:px-10 lg:px-14 lg:py-14">
      ...
      <div className="hidden lg:block">
        <p className="font-display text-[34px] leading-[1.12] tracking-[-0.02em] text-white">
      ...
      <p className="hidden font-mono text-[11px] uppercase tracking-[0.16em] text-white/35 lg:block">
        Kathmandu · Nepal
      </p>
    </aside>
    ...
    <main className="flex items-center justify-center px-5 py-12 sm:px-8 lg:px-14">
      <div className="w-full max-w-[400px]">
        <h1 className="text-[32px] sm:text-[36px]">{title}</h1>
        {subtitle && <p className="mt-3 text-[15px] leading-relaxed text-ink-soft">{subtitle}</p>}
        <div className="mt-9">{children}</div>
        {footer && <div className="mt-8 border-t border-line pt-6 text-[14px]">{footer}</div>}
      </div>
    </main>
  </div>
  )
...
export const Divider = ({ children = "or" }) => (
  <div className="my-6 flex items-center gap-4">
    ...
    <span className="font-mono text-[11px] uppercase tracking-[0.16em] text-ink-mute">{children}</span>
    <span className="h-px flex-1 bg-line" />
  </div>
)
...
export const FormError = ({ children }) =>
  children ? (
    <p
      role="alert"
      className="mb-5 rounded-md border border-red-200 bg-red-50 px-3.5 py-2.5 text-[13.5px] leading-relaxed text-red-700"
    >
      {children}
    </p>
  ) : null
