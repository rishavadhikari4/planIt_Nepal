...
const AdminDecorations = () => {
  ...
  const handleDelete = async (id) => {
    ...
    if (result.success) {
          ...
          setDecorations(decorations.filter(decoration => decoration._id !== id));
  } else {
          ...
          <div key={decoration._id} className="card decoration-card">
            ...
            <p className="decoration-card__description">{decoration.description}</p>  
              <div className="decoration-card__actions">
                <button onClick={() => navigate(`/admin-decorations/edit/${decoration._id}`)} className='edit-btn'>
                      Edit
                </button>

                <button onClick={() => handleDelete(decoration._id)} className="delete-btn">Delete</button>
              </div>        
          </div>
        ))}
      </div>
